...
while True:
    ...
    fire = fire_cascade.detectMultiScale(frame, 12, 5)  # test for fire detection
    for (x, y, w, h) in fire:
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)  # highlight the area of image with fire
    ...
    if spots_status[3]:
        spot_int = 0
    update_parking_status(4, spot_int)
    spot_int = 1
    if spots_status[2]:
        spot_int = 0
    update_parking_status(5, spot_int)
    spot_int = 1
    if spots_status[1]:
        spot_int = 0
    update_parking_status(6, spot_int)
    spot_int = 1
    if spots_status[0]:
        spot_int = 0
    update_parking_status(7, spot_int)
