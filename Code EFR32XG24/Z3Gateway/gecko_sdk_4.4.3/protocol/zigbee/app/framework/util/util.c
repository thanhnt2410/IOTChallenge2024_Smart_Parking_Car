...
static uint8_t /*enum EmberAfRetryOverride*/emberAfApsRetryOverride =
    EMBER_AF_RETRY_OVERRIDE_NONE;
static uint8_t /*enum EmberAfDisableDefaultResponse*/sli_zigbee_af_disable_default_response =
    EMBER_AF_DISABLE_DEFAULT_RESPONSE_NONE;
static uint8_t /*enum EmberAfDisableDefaultResponse*/sli_zigbee_af_saved_disable_default_response_vale =
    EMBER_AF_DISABLE_DEFAULT_RESPONSE_NONE;
...
static const uint8_t emberAfAnalogDiscreteThresholds[] =
  { 0x07, EMBER_AF_DATA_TYPE_NONE, 0x1F, EMBER_AF_DATA_TYPE_DISCRETE, 0x2F,
      EMBER_AF_DATA_TYPE_ANALOG, 0x37, EMBER_AF_DATA_TYPE_DISCRETE, 0x3F,
      EMBER_AF_DATA_TYPE_ANALOG, 0x57, EMBER_AF_DATA_TYPE_DISCRETE, 0xDF,
      EMBER_AF_DATA_TYPE_NONE, 0xE7, EMBER_AF_DATA_TYPE_ANALOG, 0xFF,
      EMBER_AF_DATA_TYPE_NONE };

uint8_t sli_zigbee_af_extended_pan_id[EXTENDED_PAN_ID_SIZE] =
  { 0, 0, 0, 0, 0, 0, 0, 0, };
...
static EmberAfDifferenceType
getDiffFloat (EmberAfDifferenceType value1, EmberAfDifferenceType value2)
{
...
emberAfGetDifference (uint8_t *pData, EmberAfDifferenceType value,
                      uint8_t dataSize, EmberAfAttributeType dataType)
{
...
static void
prepareForResponse (const EmberAfClusterCommand *cmd)
{
  ...
  if (cmd->interPanHeader == NULL)
    ...
    {
      emberAfResponseType |= ZCL_UTIL_RESP_INTERPAN;
      MEMMOVE (&interpanResponseHeader, cmd->interPanHeader,
...
// ****************************************
void
emberAfInit (uint8_t init_level)
{
  (void) init_level;
  ...
  emberAfSetExternalBuffer (appResponseData, EMBER_AF_RESPONSE_BUFFER_LEN,
                            &appResponseLength, &emberAfResponseApsFrame);
...
uint16_t
emberAfFindClusterNameIndexWithMfgCode (uint16_t cluster, uint16_t mfgCode)
{
  uint16_t index = 0u;
  while (zclClusterNames[index].id != ZCL_NULL_CLUSTER_ID)
    {
      ...
      // compatibility reasons
          && (cluster < 0xFC00u || zclClusterNames[index].mfgCode == mfgCode
...
uint16_t
emberAfFindClusterNameIndex (uint16_t cluster)
{
  return emberAfFindClusterNameIndexWithMfgCode (
      cluster, EMBER_AF_NULL_MANUFACTURER_CODE);
}
...
void
emberAfDecodeAndPrintClusterWithMfgCode (uint16_t cluster, uint16_t mfgCode)
{
  ...
  if (index == 0xFFFFu)
    ...
    {
      emberAfPrint (emberAfPrintActiveArea, "(%p)",
                    zclClusterNames[index].name);
    }
}

void
emberAfDecodeAndPrintCluster (uint16_t cluster)
{
  emberAfDecodeAndPrintClusterWithMfgCode (cluster,
...
uint16_t
emberAfGetMfgCodeFromCurrentCommand (void)
{
...
static void
printIncomingZclMessage (const EmberAfClusterCommand *cmd)
{
  ...
  if (emberAfPrintReceivedMessages)
    {
      ...
      emberAfAppPrint ("RX len %d, ep %x, clus 0x%2x ", cmd->bufLen,
                       cmd->apsFrame->destinationEndpoint,
                       cmd->apsFrame->clusterId);
      emberAfAppDebugExec (
          emberAfDecodeAndPrintClusterWithMfgCode (cmd->apsFrame->clusterId,
                                                   cmd->mfgCode));
      if (cmd->mfgSpecific)
        {
          emberAfAppPrint (" mfgId %2x", cmd->mfgCode);
        }
      emberAfAppPrint (" FC %x seq %x cmd %x payload[", cmd->buffer[0], // frame control
...
static bool
dispatchZclMessage (EmberAfClusterCommand *cmd)
{
  ...
  if (index == 0xFFu || (index >= MAX_ENDPOINT_COUNT))
  ...
  else if (emberAfNetworkIndexFromEndpointIndex(index) != cmd->networkIndex)
    {
      ...
      emberAfDebugPrint (" for endpoint 0x%x due to wrong %p: ",
                         cmd->apsFrame->destinationEndpoint, "network");
              ...
              < emberAfProfileIdFromIndex(index))))
    {
      emberAfDebugPrint ("Drop cluster 0x%2x command 0x%x",
                         cmd->apsFrame->clusterId, cmd->commandId);
      emberAfDebugPrint (" for endpoint 0x%x due to wrong %p: ",
                         cmd->apsFrame->destinationEndpoint, "profile");
          ...
          cmd->apsFrame->destinationEndpoint, cmd->apsFrame->groupId))
    {
      emberAfDebugPrint ("Drop cluster 0x%2x command 0x%x",
                         cmd->apsFrame->clusterId, cmd->commandId);
      emberAfDebugPrint (" for endpoint 0x%x due to wrong %p: ",
                         cmd->apsFrame->destinationEndpoint, "group");
...
bool
emberAfProcessMessage (EmberApsFrame *apsFrame, EmberIncomingMessageType type,
                       uint8_t *message, uint16_t msgLen, EmberNodeId source,
                       InterPanHeader *interPanHeader)
{
  ...
  curCmd = staticCmd;
  if (!emberAfProcessMessageIntoZclCmd (apsFrame, type, message, msgLen, source,
                                        interPanHeader, &curCmd))
    {
      goto kickout;
    }
  ...
  if (interPanHeader == NULL)
    {
      ...
      if (emberAfDetermineIfLinkSecurityIsRequired (
          curCmd.commandId,
          true, // incoming
...
#else
          sendStatus = emberAfSendDefaultResponse (&curCmd,
      ...
      for (i = 0u; i < emberAfEndpointCount (); i++)
        {
          ...
          if (!emberAfEndpointIndexIsEnabled (i)
              || !emberAfContainsClusterWithMfgCode (endpoint,
                                                     curCmd.apsFrame->clusterId,
                                                     curCmd.mfgCode))
            {
              continue;
...
kickout: emberAfClearResponseData ();
  MEMSET (&interpanResponseHeader, 0u, sizeof(EmberAfInterpanHeader));
  sli_zigbee_af_current_command = NULL;
  return msgHandled;
}

uint8_t
emberAfNextSequence (void)
{
  return ((++emberAfSequenceNumber) & EMBER_AF_ZCL_SEQUENCE_MASK);
}

uint8_t
emberAfGetLastSequenceNumber (void)
{
  return (emberAfSequenceNumber & EMBER_AF_ZCL_SEQUENCE_MASK);
}
...
void
emberAfSetNoReplyForNextMessage (bool set)
{
  if (set)
    {
...
void
emberAfSetRetryOverride (EmberAfRetryOverride value)
{
  emberAfApsRetryOverride = value;
}

EmberAfRetryOverride
emberAfGetRetryOverride (void)
{
  return (EmberAfRetryOverride) emberAfApsRetryOverride;
}

void
sli_zigbee_af_apply_retry_override (EmberApsOption *options)
{
  if (options == NULL)
    {
      return;
...
void
emberAfSetDisableDefaultResponse (EmberAfDisableDefaultResponse value)
{
...
emberAfGetDisableDefaultResponse (void)
{
  return (EmberAfDisableDefaultResponse) sli_zigbee_af_disable_default_response;
}

void
sli_zigbee_af_apply_disable_default_response (uint8_t *frame_control)
{
  if (frame_control == NULL)
    {
      return;
    }
  else if (sli_zigbee_af_disable_default_response
      == EMBER_AF_DISABLE_DEFAULT_RESPONSE_ONE_SHOT)
    {
      sli_zigbee_af_disable_default_response =
          sli_zigbee_af_saved_disable_default_response_vale;
      *frame_control |= ZCL_DISABLE_DEFAULT_RESPONSE_MASK;
    }
  else if (sli_zigbee_af_disable_default_response
      == EMBER_AF_DISABLE_DEFAULT_RESPONSE_PERMANENT)
    {
      *frame_control |= ZCL_DISABLE_DEFAULT_RESPONSE_MASK;
    }
  else
...
emberAfSendResponseWithCallback (EmberAfMessageSentFunction callback)
{
  ...
  if (emberAfCurrentCommand() != NULL)
    {
      if ((emberAfCurrentCommand()->apsFrame->options
          & EMBER_APS_OPTION_ENCRYPTION) != 0U)
        {
          emberAfResponseApsFrame.options |= EMBER_APS_OPTION_ENCRYPTION;
        }
      if ((emberAfCurrentCommand()->apsFrame->options & EMBER_APS_OPTION_RETRY)
          != 0U)
        {
          emberAfResponseApsFrame.options |= EMBER_APS_OPTION_RETRY;
        }
...
emberAfSendImmediateDefaultResponseWithCallback (
    EmberAfStatus status, EmberAfMessageSentFunction callback)
{
  return emberAfSendDefaultResponseWithCallback (emberAfCurrentCommand(),
                                                 status, callback);
}

EmberStatus
emberAfSendImmediateDefaultResponse (EmberAfStatus status)
{
  return emberAfSendImmediateDefaultResponseWithCallback (status, NULL);
}

EmberStatus
emberAfSendDefaultResponseWithCallback (const EmberAfClusterCommand *cmd,
  ...
  if (!cmd->mfgSpecific)
    {
      (void) emberAfPutInt8uInResp (
          frameControl & (uint8_t) ~ZCL_MANUFACTURER_SPECIFIC_MASK);
    }
  else
    {
      (void) emberAfPutInt8uInResp (
...
EmberStatus
emberAfSendDefaultResponse (const EmberAfClusterCommand *cmd,
                            EmberAfStatus status)
{
...
bool
emberAfDetermineIfLinkSecurityIsRequired (uint8_t commandId, bool incoming,
                                          bool broadcast,
                                          ...
                                          EmberNodeId remoteNodeId)
{
  ...
  // always return false.
  if ((sli_zigbee_af_test_aps_security_override == APS_TEST_SECURITY_DISABLED)
  ...
  else if (sli_zigbee_af_test_aps_security_override
  ...
  if (emberAfClusterSecurityCustomCallback (profileId, clusterId, incoming,
                                            commandId))
    {
      return true;
    }
...
uint8_t
emberAfMaximumApsPayloadLength (EmberOutgoingMessageType type,
...
emberAfCopyLongString (uint8_t *dest, uint8_t *src, uint16_t size)
{
  ...
  else if ((src[0] == 0xFFu) && (src[1] == 0xFFu))
    {
      dest[0] = 0xFFu;
      dest[1] = 0xFFu;
    }
...
int8_t
emberAfCompareValues (uint8_t *val1, uint8_t *val2, uint8_t len,
                      bool signedNumber)
{
  uint8_t i, j, k;
  if (signedNumber)
          ...
          for (i = 0; i < len; i++)
            {
              j = (
                  val1 == NULL ?
                      0 : (EM_BIG_ENDIAN ? val1[i] : val1[(len - 1) - i]));
              accum1 |= j << (8 * (len - 1 - i));
...
uint8_t
emberAfGetAttributeAnalogOrDiscreteType (uint8_t dataType)
{
  uint8_t index = 0u;
...
bool
emberAfIsThisMyEui64 (EmberEUI64 eui64)
{
  EmberEUI64 myEui64;
...
uint8_t
emberAfAppendCharacters (uint8_t *zclString, uint8_t zclStringMaxLen,
                         ...
                         uint8_t appendingCharsLen)
{
  ...
  if ((zclString == NULL) || (zclStringMaxLen == 0u) || (appendingChars == NULL)
  ...
  if ((zclString[0] == 0xFFu) || (curLen >= zclStringMaxLen))
    {
      return 0u;
    }
  ...
  charsToWrite =
      (freeChars > appendingCharsLen) ? appendingCharsLen : freeChars;
...
uint32_t
emberAfGetBufferCrc (uint8_t *pbuffer, uint16_t length, uint32_t initialValue)
{
  uint16_t i;
 ...
 */
EmberStatus
sli_zigbee_af_validate_channel_pages (uint8_t page, uint8_t channel)
{
  switch (page)
    {
    case 0u:
...
void
slabAssert (const char *file, int line)
{
...
uint8_t
emberAfGetPageFrom8bitEncodedChanPg (uint8_t chanPg)
{
...
uint8_t
emberAfGetChannelFrom8bitEncodedChanPg (uint8_t chanPg)
{
  return chanPg & ENCODED_8BIT_CHANPG_CHANNEL_MASK;
}

uint8_t
emberAfMake8bitEncodedChanPg (uint8_t page, uint8_t channel)
{
...
WEAK (
    EmberAfDifferenceType
    emberAfGetDiffCallback (EmberAfDifferenceType value1,
